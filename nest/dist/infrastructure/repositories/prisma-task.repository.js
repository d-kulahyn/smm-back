...
let PrismaTaskRepository = class PrismaTaskRepository {
    ...
    toDomain(task) {
        const assignedTo = task.assignments && task.assignments.length > 0
            ...
            : undefined;
        return new task_entity_1.Task(task.id, task.title, task.projectId, task.creatorId, task.description, task.status, task.priority, assignedTo, task.attachments || [], task.completedAt, task.dueDate, task.createdAt, task.updatedAt);
    }
};
