...
let PermissionsGuard = class PermissionsGuard {
    ...
    async canActivate(context) {
        ...
        const requiredPermissions = reflector.getAllAndOverride('permissions', [
            context.getHandler(),
            context.getClass(),
        ]);
        ...
        const anyPermissions = reflector.get('any-permissions', context.getHandler());
        if (anyPermissions) {
            return anyPermissions.some(permission => userPermissions.includes(permission));
        }
        const allPermissions = reflector.get('all-permissions', context.getHandler());
        if (allPermissions) {
