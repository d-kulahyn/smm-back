...
let FileService = class FileService {
    constructor() {
        ...
        this.messagesPath = (0, path_1.join)(this.uploadPath, 'messages');
    ...
    async saveMessageFile(file) {
        ...
        const fileName = `${Date.now()}-${Math.random().toString(36).substring(2)}-${file.originalname}`;
        const filePath = (0, path_1.join)(this.messagesPath, fileName);
        await fs_1.promises.writeFile(filePath, file.buffer);
        return {
            filePath: fileName,
    ...
    async deleteMessageFile(filePath) {
        if (!filePath)
            return;
        try {
            const fullPath = (0, path_1.join)(this.messagesPath, filePath);
            await fs_1.promises.unlink(fullPath);
        }
        catch (error) {
