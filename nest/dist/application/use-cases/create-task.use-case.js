...
let CreateTaskUseCase = class CreateTaskUseCase {
    ...
    async execute(command) {
        const task = new task_entity_1.Task(crypto.randomUUID(), command.title, command.projectId, command.creatorId, command.description, enums_1.TaskStatus.pending, command.priority || enums_1.TaskPriority.MEDIUM, command.assignedTo, command.attachments, undefined, command.dueDate);
