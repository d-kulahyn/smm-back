"use strict";
var __decorate = (this && this.__decorate) || function (decorators, target, key, desc) {
    var c = arguments.length, r = c < 3 ? target : desc === null ? desc = Object.getOwnPropertyDescriptor(target, key) : desc, d;
    if (typeof Reflect === "object" && typeof Reflect.decorate === "function") r = Reflect.decorate(decorators, target, key, desc);
    else for (var i = decorators.length - 1; i >= 0; i--) if (d = decorators[i]) r = (c < 3 ? d(r) : c > 3 ? d(target, key, r) : d(target, key)) || r;
    return c > 3 && r && Object.defineProperty(target, key, r), r;
};
var __metadata = (this && this.__metadata) || function (k, v) {
    if (typeof Reflect === "object" && typeof Reflect.metadata === "function") return Reflect.metadata(k, v);
};
var __param = (this && this.__param) || function (paramIndex, decorator) {
    return function (target, key) { decorator(target, key, paramIndex); }
};
Object.defineProperty(exports, "__esModule", { value: true });
exports.CreateChatUseCase = void 0;
const common_1 = require("@nestjs/common");
const chat_entity_1 = require("../../domain/entities/chat.entity");
const chat_member_entity_1 = require("../../domain/entities/chat-member.entity");
const exceptions_1 = require("../../shared/exceptions");
const events_1 = require("../../shared/events");
const events_2 = require("../../shared/events");
const uuid = require("uuid");
let CreateChatUseCase = class CreateChatUseCase {
    constructor(chatRepository, chatMemberRepository, eventBroadcastService) {
        this.chatRepository = chatRepository;
        this.chatMemberRepository = chatMemberRepository;
        this.eventBroadcastService = eventBroadcastService;
    }
    async execute(dto) {
        if (!dto.name || dto.name.trim().length === 0) {
            throw new exceptions_1.BusinessException('Chat name is required', 'CHAT_NAME_REQUIRED');
        }
        if (dto.name.length > 100) {
            throw new exceptions_1.BusinessException('Chat name cannot exceed 100 characters', 'CHAT_NAME_TOO_LONG');
        }
        if (!dto.projectId) {
            throw new exceptions_1.BusinessException('Project ID is required', 'PROJECT_ID_REQUIRED');
        }
        const chatId = uuid.v4();
        const chat = chat_entity_1.Chat.create({
            id: chatId,
            projectId: dto.projectId,
            name: dto.name.trim(),
            description: dto.description?.trim(),
            createdBy: dto.createdBy,
            isGroup: dto.isGroup || false,
            avatar: dto.avatar,
        });
        const createdChat = await this.chatRepository.create(chat);
        const creatorMember = chat_member_entity_1.ChatMember.create({
            id: uuid.v4(),
            chatId: chatId,
            userId: dto.createdBy,
            role: chat_member_entity_1.ChatMemberRole.ADMIN,
        });
        await this.chatMemberRepository.create(creatorMember);
        if (dto.members && dto.members.length > 0) {
            for (const memberId of dto.members) {
                if (memberId !== dto.createdBy) {
                    const member = chat_member_entity_1.ChatMember.create({
                        id: uuid.v4(),
                        chatId: chatId,
                        userId: memberId,
                        role: chat_member_entity_1.ChatMemberRole.MEMBER,
                    });
                    await this.chatMemberRepository.create(member);
                }
            }
        }
        const event = new events_2.ChatCreatedEvent(createdChat, dto.createdBy, dto.projectId);
        this.eventBroadcastService.broadcast(event).catch(error => {
            console.error('Failed to broadcast ChatCreated event:', error);
        });
        return createdChat;
    }
};
exports.CreateChatUseCase = CreateChatUseCase;
exports.CreateChatUseCase = CreateChatUseCase = __decorate([
    (0, common_1.Injectable)(),
    __param(0, (0, common_1.Inject)('CHAT_REPOSITORY')),
    __param(1, (0, common_1.Inject)('CHAT_MEMBER_REPOSITORY')),
    __metadata("design:paramtypes", [Object, Object, events_1.EventBroadcastService])
], CreateChatUseCase);
//# sourceMappingURL=create-chat.use-case.js.map