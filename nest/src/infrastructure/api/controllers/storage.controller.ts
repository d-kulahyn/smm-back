...
export class StorageController {
  ...
  constructor(private readonly chunkedFileService: ChunkedFileService) {}
  ...
  async createFile(
    ...
    try {
      ...
      let thumbnail = await this.chunkedFileService.createFile({
        originalName: `thumbnail_${createFileDto.originalName}`,
      ...
      // Используем FileResource для формирования ответа
      return FileResource.fromEntity(fileEntity, undefined, thumbnail);
    } catch (error) {
  ...
  @ApiOperation({
    ...
    description:
      'Upload a chunk of a file (for large files). Send file data in "chunk" field.',
  })
  @ApiParam({
    name: "fileId",
    description: "File ID",
    ...
    try {
      if (!file) {
        throw new BadRequestException(
          'No chunk data provided. Please send file data in "chunk" field.',
        );
      }
  ...
  @ApiOperation({
    ...
    description:
      "Upload a chunk of a file with flexible field name. Accepts any file field name.",
  })
  @ApiParam({
    name: "fileId",
    description: "File ID",
    ...
    try {
      ...
      return new Promise((resolve, reject) => {
        upload(req, res, async (err) => {
          if (err) {
            return reject(
              new BadRequestException(`Upload error: ${err.message}`),
            );
          }
  ...
  async completeFileUpload(
    ...
    try {
      const completedFile =
        await this.chunkedFileService.completeUpload(fileId);
...
console.log(filePath);
  ...
  @ApiOperation({
    ...
    description:
      "Get all files attached to an entity with optional filtering by device ID",
  })
  @ApiParam({
    name: "entityType",
  ...
  async getFilesByEntity(
    ...
    try {
      let files = await this.chunkedFileService.getFilesByEntity(
        entityType,
        entityId,
      );

      // Фильтруем по deviceId если передан
      if (deviceId) {
        files = files.filter((file) => file.deviceId === deviceId);
      }

      // Загружаем все миниатюры для файлов
      const thumbnails =
        await this.chunkedFileService.loadThumbnailsForFiles(files);

      // Загружаем все группы файлов
      const fileGroupIds = files
        .map((file) => file.fileGroupId)
        .filter((id): id is string => !!id);

      const fileGroups = new Map();
      if (fileGroupIds.length > 0) {
        const groups =
          await this.chunkedFileService["fileGroupRepository"].findByIds(
            fileGroupIds,
          );
        groups.forEach((group) => fileGroups.set(group.id, group));
      }

      // Используем статический метод collection для преобразования
      const fileResources = FileResource.collection(
        files,
        fileGroups,
        thumbnails,
      );
  ...
  @ApiOperation({
    ...
    description:
      "Upload a specific chunk by index for parallel uploads. Chunks can be uploaded in any order.",
  })
  @ApiParam({
    name: "fileId",
    description: "File ID",
  ...
  async uploadFileChunkWithIndex(
    ...
    try {
      ...
      if (isNaN(chunkIndex) || chunkIndex < 0) {
        throw new BadRequestException(
          "Invalid chunk index. Must be a non-negative integer.",
        );
      }
  ...
  @ApiOperation({
    ...
    description:
      "Get information about chunk upload progress including missing chunks",
  })
  @ApiParam({
    name: "fileId",
    description: "File ID",
  ...
  async getChunkInfo(
    ...
    try {
      ...
      throw new BadRequestException(
        error.message || "Failed to get chunk info",
      );
    }
  }
  ...
  async assignFileToGroup(
    ...
    const { AssignFileToGroupUseCase } = await import(
      "../../../application/use-cases/manage-file-groups.use-case"
    );
  ...
  async removeFileFromGroup(
    ...
    const { RemoveFileFromGroupUseCase } = await import(
      "../../../application/use-cases/manage-file-groups.use-case"
    );
  ...
  async getFilesByGroup(
    ...
    // Загружаем группу файлов
    const fileGroups = new Map();
    const fileGroup =
      await this.chunkedFileService["fileGroupRepository"].findById(groupId);
    if (fileGroup) {
      fileGroups.set(fileGroup.id, fileGroup);
    }
